...
namespace Engine
{
	...
	VulkanSamplerCreateInfo::VulkanSamplerCreateInfo(const Sampler* sampler)
		: address_u(get_type(sampler->address_u)), address_v(get_type(sampler->address_v)),
		  address_w(get_type(sampler->address_w)), compare_func(get_type(sampler->compare_func)),
		  anisotropy(sampler->anisotropy), mip_lod_bias(sampler->mip_lod_bias), min_lod(sampler->min_lod),
		  max_lod(sampler->max_lod), unnormalized_coordinates(sampler->unnormalized_coordinates),
	...
	{
		vk::SamplerCreateInfo sampler_info({}, info.mag_filter, info.min_filter, info.mipmap_mode, info.address_u,
										   info.address_v, info.address_w, info.mip_lod_bias,
