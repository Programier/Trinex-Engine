...
namespace Engine
{
	...
	static constexpr inline vk::AttachmentDescription
	create_attachment_desctiption(vk::Format format, vk::ImageLayout layout, bool has_stencil)
	{
		return vk::AttachmentDescription(vk::AttachmentDescriptionFlags(), format, vk::SampleCountFlagBits::e1,
										 ...
										 has_stencil ? vk::AttachmentStoreOp::eStore
	...
	VulkanRenderPass& VulkanRenderPass::init(const Span<RenderSurface*>& color_attachments,
											 RenderSurface* depth_stencil)
	{
	...
	VulkanRenderPass& VulkanRenderPass::create()
	{
		...
		m_render_pass = API->m_device.createRenderPass(vk::RenderPassCreateInfo(
