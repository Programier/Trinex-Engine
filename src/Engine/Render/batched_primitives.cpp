...
namespace Trinex
{
	...
	BatchedLines& BatchedLines::flush(RHIContext* ctx, Renderer* renderer)
	{
		...
		ctx->bind_vertex_attribute(RHISemantic::Position, RHIVertexFormat::RGB32F, 0, 0);
		ctx->bind_vertex_attribute(RHISemantic::Color, RHIVertexFormat::RGBA8, 0, 12);
		ctx->bind_vertex_attribute(RHISemantic::UserData, RHIVertexFormat::R32F, 0, 16);
