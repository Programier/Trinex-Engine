...
private:
    ...
    std::string _M_ip;
    int _M_port;
    Array<char, BUFFER_SIZE> _M_buffer;
    ...
    std::function<void(Array<char, BUFFER_SIZE>&, int&)> _M_function;


public:
    Server();
    Server(int port, const std::string& ip, const std::function<void(Array<char, BUFFER_SIZE>&, int&)>& _function);
    ...
    const Array<char, BUFFER_SIZE>& buffer() const;
