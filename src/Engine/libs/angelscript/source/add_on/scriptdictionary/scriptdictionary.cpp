...
{
    dictMap_t::iterator it;
    for (it = dict.begin(); it != dict.end(); it++) it->second.FreeValue(engine);
...
bool CScriptDictValue::Get(asIScriptEngine* engine, void* value, int typeId) const
{
    ...
    if (typeId & asTYPEID_OBJHANDLE)
    {
        ...
        if ((m_typeId & asTYPEID_MASK_OBJECT))
        {
            ...
            engine->RefCastObject(m_valueObj, engine->GetTypeInfoById(m_typeId), engine->GetTypeInfoById(typeId),
...
void RegisterScriptDictionary_Native(asIScriptEngine* engine)
{
    ...
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("dictionary", asBEHAVE_ADDREF, "void f()", asMETHOD(CScriptDictionary, AddRef),
                                        asCALL_THISCALL);
    assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("dictionary", "bool get(const string &in, ?&out) const",
                                     asMETHODPR(CScriptDictionary, Get, (const dictKey_t&, void*, int) const, bool),
                                     asCALL_THISCALL);
    assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("dictionary", "uint getSize() const", asMETHOD(CScriptDictionary, GetSize),
                                     asCALL_THISCALL);
    assert(r >= 0);
    r = engine->RegisterObjectMethod("dictionary", "bool delete(const string &in)", asMETHOD(CScriptDictionary, Delete),
                                     asCALL_THISCALL);
    assert(r >= 0);
    ...
    r = engine->RegisterObjectMethod("dictionary", "dictionaryValue &opIndex(const string &in)",
                                     asMETHODPR(CScriptDictionary, operator[], (const dictKey_t&), CScriptDictValue*),
                                     asCALL_THISCALL);
    assert(r >= 0);
    r = engine->RegisterObjectMethod(
    ...
    assert(r >= 0);
    // Same as delete
    r = engine->RegisterObjectMethod("dictionary", "void erase(const string &in)", asMETHOD(CScriptDictionary, Delete),
                                     asCALL_THISCALL);
    assert(r >= 0);
    // Same as deleteAll
...
void RegisterScriptDictionary_Generic(asIScriptEngine* engine)
{
    ...
    assert(r >= 0);
    r = engine->RegisterObjectMethod("dictionary", "bool isEmpty() const", asFUNCTION(ScriptDictionaryIsEmpty_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
    r = engine->RegisterObjectMethod("dictionary", "uint getSize() const", asFUNCTION(ScriptDictionaryGetSize_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
    ...
    assert(r >= 0);
    r = engine->RegisterObjectMethod("dictionary", "void deleteAll()", asFUNCTION(ScriptDictionaryDeleteAll_Generic),
                                     asCALL_GENERIC);
    assert(r >= 0);
