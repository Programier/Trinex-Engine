...
int CompareRelation(asIScriptEngine* engine, void* lobj, void* robj, int typeId, int& result)
{
    ...
    if (ti)
    {
        ...
        for (asUINT n = 0; n < ti->GetMethodCount(); n++)
        {
            ...
            asDWORD flags;
            if (strcmp(f->GetName(), "opCmp") == 0 && f->GetReturnTypeId(&flags) == asTYPEID_INT32 &&
...
int CompareEquality(asIScriptEngine* engine, void* lobj, void* robj, int typeId, bool& result)
{
    ...
    if (ti)
    {
        ...
        for (asUINT n = 0; n < ti->GetMethodCount(); n++)
        {
            asIScriptFunction* f = ti->GetMethodByIndex(n);
            asDWORD flags;
            if (strcmp(f->GetName(), "opEquals") == 0 && f->GetReturnTypeId(&flags) == asTYPEID_BOOL &&
                flags == asTM_NONE && f->GetParamCount() == 1)
            {
                int paramTypeId;
                f->GetParam(0, &paramTypeId, &flags);
...
int WriteConfigToStream(asIScriptEngine* engine, ostream& strm)
{
    ...
    struct TypeWriter {
        static void Write(asIScriptEngine* engine, ostream& strm, asITypeInfo* type, string& currNamespace,
                          asDWORD& currAccessMask)
        {
            const char* nameSpace = type->GetNamespace();
            if (nameSpace != currNamespace)
            {
            ...
            else
            {
                ...
                for (m = 0; m < type->GetBehaviourCount(); m++)
                {
                    ...
                    if (beh == asBEHAVE_CONSTRUCT)
                        // Prefix 'void'
                        strm << "objbeh \"" << typeDecl.c_str() << "\" " << beh << " \"void "
                             << Escape::Quotes(func->GetDeclaration(false)).c_str() << "\"\n";
    ...
    if (typeId > 0)
        strm << "strfactory \"" << ((flags & asTM_CONST) ? "const " : "") << engine->GetTypeDeclaration(typeId)
             << ((flags & asTM_INOUTREF) ? "&" : "") << "\"\n";
    ...
    while (pos < config.length())
    {
        ...
        if (token == "ep")
        {
            ...
            if (ep != asEP_COPY_SCRIPT_SECTIONS && ep != asEP_MAX_STACK_SIZE &&
                ep != asEP_INIT_GLOBAL_VARS_AFTER_BUILD && ep != asEP_EXPAND_DEF_ARRAY_TO_TMPL &&
        ...
        else if (token == "objtype")
        {
            ...
            r = engine->RegisterObjectType(name.c_str(), (atol(flags.c_str()) & asOBJ_VALUE) ? 1 : 0,
                                           atol(flags.c_str()));
            if (r < 0)
            {
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_ERROR,
        ...
        else if (token == "objbeh")
        {
            ...
            if (behave == asBEHAVE_TEMPLATE_CALLBACK)
            {
                ...
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_WARNING,
                                     "Cannot register template callback without the actual implementation");
            }
            else
            {
        ...
        else if (token == "objprop")
        {
            ...
            r = engine->RegisterObjectProperty(name.c_str(), decl.c_str(), type->GetPropertyCount(),
                                               compositeOffset != "0" ? type->GetPropertyCount() : 0,
                                               isCompositeIndirect != "0");
            if (r < 0)
            {
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_ERROR,
        ...
        else if (token == "prop")
        {
            ...
            r = engine->RegisterGlobalProperty(decl.c_str(),
                                               reinterpret_cast<void*>(asPWORD(engine->GetGlobalPropertyCount() + 1)));
            if (r < 0)
            {
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_ERROR,
        ...
        else if (token == "strfactory")
        {
            ...
            {
                engine->WriteMessage(configFile, in::GetLineNumber(config, pos), 0, asMSGTYPE_WARNING,
                                     "Cannot register string factory without the actual implementation");
                return -1;
            }
            else
            {
    ...
    if (showStack)
    {
        ...
        for (asUINT n = 1; n < ctx->GetCallstackSize(); n++)
        {
            ...
            if (function)
            {
                ...
                {
                    text << (function->GetScriptSectionName() ? function->GetScriptSectionName() : "") << " ("
...
void RegisterExceptionRoutines(asIScriptEngine* engine)
{
    ...
    {
        r = engine->RegisterGlobalFunction("void throw(const string &in)", asFUNCTION(ScriptThrow), asCALL_CDECL);
        assert(r >= 0);
        r = engine->RegisterGlobalFunction("string getExceptionInfo()", asFUNCTION(ScriptGetExceptionInfo),
                                           asCALL_CDECL);
        assert(r >= 0);
    }
    else
    {
        r = engine->RegisterGlobalFunction("void throw(const string &in)", WRAP_FN(ScriptThrow), asCALL_GENERIC);
        assert(r >= 0);
        r = engine->RegisterGlobalFunction("string getExceptionInfo()", WRAP_FN(ScriptGetExceptionInfo),
                                           asCALL_GENERIC);
        assert(r >= 0);
    }
}
