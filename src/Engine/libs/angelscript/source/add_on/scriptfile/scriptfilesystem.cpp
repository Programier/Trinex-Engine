...
void RegisterScriptFileSystem_Native(asIScriptEngine* engine)
{
    ...
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("filesystem", asBEHAVE_ADDREF, "void f()", asMETHOD(CScriptFileSystem, AddRef),
                                        asCALL_THISCALL);
    assert(r >= 0);
...
void RegisterScriptFileSystem_Generic(asIScriptEngine* engine)
{
    ...
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("filesystem", asBEHAVE_ADDREF, "void f()", WRAP_MFN(CScriptFileSystem, AddRef),
                                        asCALL_GENERIC);
    assert(r >= 0);
    r = engine->RegisterObjectBehaviour("filesystem", asBEHAVE_RELEASE, "void f()",
...
CScriptFileSystem::CScriptFileSystem()
{
    ...
    for (string::size_type n = 0; n < currentPath.size(); n++)
        if (currentPath[n] == '\\')
            currentPath[n] = '/';
}
...
bool CScriptFileSystem::ChangeCurrentPath(const string& path)
{
    ...
    // Replace \ with / for consistency
    for (string::size_type n = 0; n < newPath.size(); n++)
        if (newPath[n] == '\\')
            newPath[n] = '/';
