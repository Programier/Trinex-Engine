...
struct asSFuncPtr {
    ...
    {
        for (size_t n = 0; n < sizeof(ptr.dummy); n++) ptr.dummy[n] = 0;
        flag = f;
    }

    void CopyMethodPtr(const void* mthdPtr, size_t size)
    {
        for (size_t n = 0; n < size; n++) ptr.dummy[n] = reinterpret_cast<const char*>(mthdPtr)[n];
    }
...
struct asSFuncPtr {
    asSFuncPtr(asBYTE f)
    {
        for (int n = 0; n < sizeof(ptr.dummy); n++) ptr.dummy[n] = 0;
        flag = f;
    }
...
public:
    ...
    virtual int SetStateRegisters(asUINT stackLevel, asIScriptFunction* callingSystemFunction,
                                  ...
                                  asQWORD valueRegister, void* objectRegister, asITypeInfo* objectTypeRegister) = 0;
    virtual int SetCallStateRegisters(asUINT stackLevel, asDWORD stackFramePointer, asIScriptFunction* currentFunction,
                                      asDWORD programPointer, asDWORD stackPointer, asDWORD stackIndex)         = 0;
...
public:
    ...
    virtual int GetProperty(asUINT index, const char** name, int* typeId = 0, bool* isPrivate = 0,
                            ...
                            int* compositeOffset = 0, bool* isCompositeIndirect = 0) const        = 0;
