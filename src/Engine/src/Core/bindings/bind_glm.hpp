...
#define implement_vector_wrapper(name)                                                                                 \
    ...
    static void bind_glm_behaviours(ScriptClassRegistrar& registrar, const String& prop_type)
    {
        ...
        registrar.behave(ScriptClassBehave::Construct,
                         fmt::format("void f(const {}& in)", registrar.class_base_name()).c_str(),
    ...
    static void bind_glm_operators(ScriptClassRegistrar& registrar, const String& prop_type)
    {
        ...
        registrar.opfunc(fmt::format("{}& opAssign(const {}& in)", name, name).c_str(), func_of<T&>(&T::operator=),
                         ScriptCallConv::THISCALL);

        registrar.opfunc(fmt::format("bool opEquals(const {}& in) const", name).c_str(), func_of<bool>(&T::operator==),
                         ScriptCallConv::THISCALL);
    ...
    {
        registrar.opfunc(fmt::format("{} opIndex(uint index) const", const_type).c_str(),
                         func_of<ConstType>(&T::value_at), ScriptCallConv::THISCALL);
        registrar.opfunc(fmt::format("{} opIndex(uint index)", type).c_str(), func_of<RefType>(&T::value_at),
                         ScriptCallConv::THISCALL);
    }
