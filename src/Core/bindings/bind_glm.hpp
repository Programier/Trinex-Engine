...
#define implement_vector_wrapper(name)                                                                                 \
	...
	static void bind_glm_operators(ScriptClassRegistrar& registrar, const String& prop_type)
	{
		...
		registrar.opfunc(fmt::format("bool opEquals(const {}& in) const", name).c_str(),
						 method_of<bool>(&T::operator==), ScriptCallConv::ThisCall);
	...
	void bind_index_op(ScriptClassRegistrar& registrar, const char* const_type, const char* type)
	{
		registrar.opfunc(fmt::format("{} opIndex(uint index) const", const_type).c_str(),
						 method_of<ConstType>(&T::value_at), ScriptCallConv::ThisCall);
