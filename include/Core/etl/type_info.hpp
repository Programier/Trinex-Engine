...
namespace Engine
{
	...
	struct ENGINE_EXPORT type_info_base{protected: template<std::size_t... indices> static consteval auto substring_as_array(
		    std::string_view str, std::index_sequence<indices...>){return std::array{str[indices]..., '\0'};
}// namespace Engine
...
public:
	...
	static consteval std::string_view name() { return m_name.data(); }
