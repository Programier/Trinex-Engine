...
namespace Engine
{
	...
	struct Serializer<ForwardList<T>> {
		bool serialize(Archive& ar, ForwardList<T>& list) { return trinex_serialize_list(ar, list); }
	};

	template<typename T>
	struct Serializer<List<T>> {
		bool serialize(Archive& ar, List<T>& list) { return trinex_serialize_list(ar, list); }
	};
}// namespace Engine
