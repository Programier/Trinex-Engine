...
#define debug_log(tag, format, ...) Trinex::Logger::logger->debug(tag, format __VA_OPT__(, ##__VA_ARGS__))
#else
#define debug_log(tag, format, ...)
#endif
#define warn_log(tag, format, ...) Trinex::Logger::logger->warning(tag, format __VA_OPT__(, ##__VA_ARGS__))
#define info_log(tag, format, ...) Trinex::Logger::logger->log(tag, format __VA_OPT__(, ##__VA_ARGS__))
#define error_log(tag, format, ...) Trinex::Logger::logger->error(tag, format __VA_OPT__(, ##__VA_ARGS__))
}// namespace Trinex
