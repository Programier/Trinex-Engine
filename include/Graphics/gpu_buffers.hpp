...
namespace Engine
{
	...
	{
	private:
		RenderResourcePtr<RHI_Buffer> m_buffer;
		byte* m_data              = nullptr;
		uint32_t m_vtx_count      = 0;
		uint16_t m_stride         = 0;
		BufferCreateFlags m_flags = BufferCreateFlags::Static;

	public:
		VertexBufferBase();
		// clang-format off
		VertexBufferBase(BufferCreateFlags type, uint16_t stride, size_t count, const void* data = nullptr, bool keep_cpu_data = false);
		...
		VertexBufferBase& init(BufferCreateFlags type, size_t stride, size_t count, const void* data = nullptr, bool keep_cpu_data = false);
		// clang-format on

		VertexBufferBase& init(bool keep_cpu_data = false);
		byte* allocate_data(BufferCreateFlags type, uint16_t stride, size_t count);
	...
	public:
		...
		VertexBuffer(const std::initializer_list<T>& list, BufferCreateFlags type = BufferCreateFlags::Static,
		             bool keep_cpu_data = false)
		    : VertexBufferBase(type, sizeof(T), list.size(), reinterpret_cast<const byte*>(list.begin()), keep_cpu_data)
		{}

		VertexBuffer(BufferCreateFlags type, size_t count, const T* data = nullptr, bool keep_cpu_data = false)
		    : VertexBufferBase(type, sizeof(T), count, reinterpret_cast<const byte*>(data), keep_cpu_data)
		{}

		VertexBuffer(const T* begin, const T* end, BufferCreateFlags type = BufferCreateFlags::Static, bool keep_cpu_data = false)
		    : VertexBufferBase(type, sizeof(T), end - begin, reinterpret_cast<const byte*>(begin), keep_cpu_data)
		{}

		inline VertexBuffer& init(BufferCreateFlags type, size_t count, const T* data = nullptr, bool keep_cpu_data = false)
		{
		...
		inline T* allocate_data(BufferCreateFlags type, size_t size)
		{
	...
	public:
		IndexBuffer();
		// clang-format off
		IndexBuffer(BufferCreateFlags type, size_t count, const uint16_t* data = nullptr, bool keep_cpu_data = false);
		IndexBuffer(BufferCreateFlags type, size_t count, const uint32_t* data = nullptr, bool keep_cpu_data = false);
		IndexBuffer(const std::initializer_list<uint16_t>& list, BufferCreateFlags type = BufferCreateFlags::Static, bool keep_cpu_data = false);
		IndexBuffer(const std::initializer_list<uint32_t>& list, BufferCreateFlags type = BufferCreateFlags::Static, bool keep_cpu_data = false);
		IndexBuffer(const uint16_t* start, const uint16_t* end, BufferCreateFlags type = BufferCreateFlags::Static, bool keep_cpu_data = false);
		IndexBuffer(const uint32_t* start, const uint32_t* end, BufferCreateFlags type = BufferCreateFlags::Static, bool keep_cpu_data = false);
		...
		IndexBuffer& init(BufferCreateFlags type, size_t count, const uint16_t* data = nullptr, bool keep_cpu_data = false);
		IndexBuffer& init(BufferCreateFlags type, size_t count, const uint32_t* data = nullptr, bool keep_cpu_data = false);
		IndexBuffer& init(bool keep_cpu_data = false);
		byte* allocate_data(BufferCreateFlags type, RHIIndexFormat format, size_t count);
		IndexBuffer& release();
	...
	public:
		...
		TypedIndexBuffer(BufferCreateFlags type, size_t count, const T* data = nullptr, bool keep_cpu_data = false)
		    : IndexBuffer(type, size, data, keep_cpu_data)
		{}

		TypedIndexBuffer(const std::initializer_list<T>& list, BufferCreateFlags type = BufferCreateFlags::Static,
		                 bool keep_cpu_data = false)
		    : IndexBuffer(list, type, keep_cpu_data)
		{}

		TypedIndexBuffer(const T* start, const T* end, BufferCreateFlags type = BufferCreateFlags::Static,
		                 bool keep_cpu_data = false)
		    : IndexBuffer(start, end, type, keep_cpu_data)
		{}

		TypedIndexBuffer& init(BufferCreateFlags type, size_t count, const T* data = nullptr, bool keep_cpu_data = false)
		{
		...
		T* allocate_data(BufferCreateFlags type, size_t count)
		{
