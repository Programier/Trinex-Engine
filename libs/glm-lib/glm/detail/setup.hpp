...
#define GLM_CLIP_CONTROL_ZO_BIT		(1 << 0) // ZERO_TO_ONE depth range (e.g. Vulkan, DirectX)
#define GLM_CLIP_CONTROL_NO_BIT		(1 << 1) // NEGATIVE_ONE_TO_ONE depth range (OpenGL default)
#define GLM_CLIP_CONTROL_LH_BIT		(1 << 2) // LEFT_HANDED coordinate system (Vulkan, DirectX, Metal)
#define GLM_CLIP_CONTROL_RH_BIT		(1 << 3) // RIGHT_HANDED coordinate system (OpenGL default)
...
#ifndef GLM_CLIP_SPACE_FLIP_Y
#	define GLM_CLIP_SPACE_FLIP_Y GLM_DISABLE
#endif

#if GLM_CLIP_SPACE_FLIP_Y
#	define GLM_CLIP_SPACE_Y_AXIS_SIGN(x) -(x)
#else
#	define GLM_CLIP_SPACE_Y_AXIS_SIGN(x) x
#endif
