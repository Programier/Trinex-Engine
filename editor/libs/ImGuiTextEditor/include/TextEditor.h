...
namespace ImGui
{
    ...
    public:
        ...
        struct Shortcut {
            ...
            Shortcut(ImGuiKey vk1 = ImGuiKey_None, bool alt = 0, bool ctrl = 0, bool shift = 0)
                : Alt(alt), Ctrl(ctrl), Shift(shift), Key(vk1)
            {}
        };
        ...
        struct Identifier {
            Identifier()
            {}
            Identifier(const std::string& declr) : mDeclaration(declr)
            {}
        ...
        inline const std::vector<Breakpoint>& GetBreakpoints()
        {
            return mBreakpoints;
        }
        void SetCurrentLineIndicator(int line, bool displayBar = true);
        inline int GetCurrentLineIndicator()
        {
            return mDebugCurrentLine;
        }

        inline bool IsDebugging()
        {
            return mDebugCurrentLine > 0;
        }
        ...
        inline void SetHighlightedLines(const std::vector<int>& lines)
        {
            mHighlightedLines = lines;
        }
        inline void ClearHighlightedLines()
        {
            mHighlightedLines.clear();
        }

        inline void SetTabSize(int s)
        {
            mTabSize = std::max<int>(0, std::min<int>(32, s));
        }
        inline int GetTabSize()
        {
            return mTabSize;
        }

        inline void SetInsertSpaces(bool s)
        {
            mInsertSpaces = s;
        }
        inline int GetInsertSpaces()
        {
            return mInsertSpaces;
        }

        inline void SetSmartIndent(bool s)
        {
            mSmartIndent = s;
        }
        inline void SetAutoIndentOnPaste(bool s)
        {
            mAutoindentOnPaste = s;
        }
        inline void SetHighlightLine(bool s)
        {
            mHighlightLine = s;
        }
        inline void SetCompleteBraces(bool s)
        {
            mCompleteBraces = s;
        }
        inline void SetHorizontalScroll(bool s)
        {
            mHorizontalScroll = s;
        }
        inline void SetSmartPredictions(bool s)
        {
            mAutocomplete = s;
        }
        inline void SetFunctionDeclarationTooltip(bool s)
        {
            mFunctionDeclarationTooltipEnabled = s;
        }
        inline void SetFunctionTooltips(bool s)
        {
            mFuncTooltips = s;
        }
        inline void SetActiveAutocomplete(bool cac)
        {
            mActiveAutocomplete = cac;
        }
        inline void SetScrollbarMarkers(bool markers)
        {
            mScrollbarMarkers = markers;
        }
        inline void SetSidebarVisible(bool s)
        {
            mSidebar = s;
        }
        inline void SetSearchEnabled(bool s)
        {
            mHasSearch = s;
        }
        inline void SetHiglightBrackets(bool s)
        {
            mHighlightBrackets = s;
        }
        inline void SetFoldEnabled(bool s)
        {
            mFoldEnabled = s;
        }
        ...
        inline void SetUIFontSize(float size)
        {
            mUIFontSize = size;
        }
        inline void SetEditorFontSize(float size)
        {
            mEditorFontSize = size;
        }
        ...
        inline int GetTextStart() const
        {
            return mShowLineNumbers ? 7 : 3;
        }
        ...
        inline const std::unordered_map<std::string, Function>& GetAutocompleteFunctions()
        {
            return mACFunctions;
        }
        inline const std::unordered_map<std::string, std::vector<Variable>>& GetAutocompleteUserTypes()
        {
            return mACUserTypes;
        }
        inline const std::vector<Variable>& GetAutocompleteUniforms()
        {
            return mACUniforms;
        }
        inline const std::vector<Variable>& GetAutocompleteGlobals()
        {
            return mACGlobals;
        }
    ...
    private:
        ...
        std::string
                mACObject;// if mACObject is not empty, it means user typed '.' -> suggest struct members and methods for mACObject
