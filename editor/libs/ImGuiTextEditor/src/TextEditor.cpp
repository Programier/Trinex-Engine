...
namespace ImGui
{
    ...
    TextEditor::TextEditor()
        : mLineSpacing(1.0f), mUndoIndex(0), mInsertSpaces(false), mTabSize(4), mHighlightBrackets(false), mAutocomplete(true),
          mACOpened(false), mHighlightLine(true), mHorizontalScroll(true), mCompleteBraces(true), mShowLineNumbers(true),
          mSmartIndent(true), mOverwrite(false), mReadOnly(false), mWithinRender(false), mScrollToCursor(false),
          mScrollToTop(false), mTextChanged(false), mColorizerEnabled(true), mTextStart(20.0f),
          mLeftMargin(DebugDataSpace + LineNumberSpace), mCursorPositionChanged(false), mColorRangeMin(0), mColorRangeMax(0),
          mSelectionMode(SelectionMode::Normal), mCheckComments(true), mLastClick(-1.0f), mHandleKeyboardInputs(true),
          mHandleMouseInputs(true), mIgnoreImGuiChild(false), mShowWhitespaces(false), mDebugBar(false),
          mDebugCurrentLineUpdated(false), mDebugCurrentLine(-1), mPath(""), OnContentUpdate(nullptr), mFuncTooltips(true),
          mUIScale(1.0f), mUIFontSize(18.0f), mEditorFontSize(18.0f), mActiveAutocomplete(false), m_readyForAutocomplete(false),
          m_requestAutocomplete(false), mScrollbarMarkers(false), mAutoindentOnPaste(false), mFunctionDeclarationTooltip(false),
          mFunctionDeclarationTooltipEnabled(false), mIsSnippet(false), mSnippetTagSelected(0), mSidebar(true), mHasSearch(true),
          mReplaceIndex(0), mFoldEnabled(true), mFoldLastIteration(0), mFoldSorted(false), mLastScroll(0.0f),
          mStartTime(std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch())
                             .count())
    {
    ...
    const std::vector<TextEditor::Shortcut> TextEditor::GetDefaultShortcuts()
    {
        ...
        ret[(int) TextEditor::ShortcutID::Undo]       = TextEditor::Shortcut(ImGuiKey_Z, 0, 1, 0);        // CTRL+Z
        ret[(int) TextEditor::ShortcutID::Redo]       = TextEditor::Shortcut(ImGuiKey_Y, 0, 1, 0);        // CTRL+Y
        ret[(int) TextEditor::ShortcutID::MoveUp]     = TextEditor::Shortcut(ImGuiKey_UpArrow, 0, 0, 0);  // UP ARROW
        ret[(int) TextEditor::ShortcutID::SelectUp]   = TextEditor::Shortcut(ImGuiKey_UpArrow, 0, 0, 1);  // SHIFT + UP ARROW
        ret[(int) TextEditor::ShortcutID::MoveDown]   = TextEditor::Shortcut(ImGuiKey_DownArrow, 0, 0, 0);// DOWN ARROW
        ret[(int) TextEditor::ShortcutID::SelectDown] = TextEditor::Shortcut(ImGuiKey_DownArrow, 0, 0, 1);// SHIFT + DOWN ARROW
        ret[(int) TextEditor::ShortcutID::MoveLeft] =
                TextEditor::Shortcut(ImGuiKey_LeftArrow, 0, 0, 0);// LEFT ARROW (+ SHIFT/CTRL)
        ret[(int) TextEditor::ShortcutID::SelectLeft]   = TextEditor::Shortcut(ImGuiKey_LeftArrow, 0, 0, 1);// SHIFT + LEFT ARROW
        ret[(int) TextEditor::ShortcutID::MoveWordLeft] = TextEditor::Shortcut(ImGuiKey_LeftArrow, 0, 1, 0);// CTRL + LEFT ARROW
        ret[(int) TextEditor::ShortcutID::SelectWordLeft] =
                TextEditor::Shortcut(ImGuiKey_LeftArrow, 0, 1, 1);// CTRL + SHIFT + LEFT ARROW
        ret[(int) TextEditor::ShortcutID::MoveRight]   = TextEditor::Shortcut(ImGuiKey_RightArrow, 0, 0, 0);// RIGHT ARROW
        ret[(int) TextEditor::ShortcutID::SelectRight] = TextEditor::Shortcut(ImGuiKey_RightArrow, 0, 0, 1);// SHIFT + RIGHT ARROW
        ret[(int) TextEditor::ShortcutID::MoveWordRight] =
                TextEditor::Shortcut(ImGuiKey_RightArrow, 0, 1, 0);// CTRL + RIGHT ARROW
        ret[(int) TextEditor::ShortcutID::SelectWordRight] =
                TextEditor::Shortcut(ImGuiKey_RightArrow, 0, 1, 1);// CTRL + SHIFT + RIGHT ARROW
        ret[(int) TextEditor::ShortcutID::MoveUpBlock]     = TextEditor::Shortcut(ImGuiKey_PageUp, 0, 0, 0);  // PAGE UP
        ret[(int) TextEditor::ShortcutID::SelectUpBlock]   = TextEditor::Shortcut(ImGuiKey_PageUp, 0, 0, 1);  // SHIFT + PAGE UP
        ret[(int) TextEditor::ShortcutID::MoveDownBlock]   = TextEditor::Shortcut(ImGuiKey_PageDown, 0, 0, 0);// PAGE DOWN
        ret[(int) TextEditor::ShortcutID::SelectDownBlock] = TextEditor::Shortcut(ImGuiKey_PageDown, 0, 0, 1);// SHIFT + PAGE DOWN
        ret[(int) TextEditor::ShortcutID::MoveTop]         = TextEditor::Shortcut(ImGuiKey_Home, 0, 1, 0);    // CTRL + HOME
        ret[(int) TextEditor::ShortcutID::SelectTop]       = TextEditor::Shortcut(ImGuiKey_Home, 0, 1, 1);  // CTRL + SHIFT + HOME
        ret[(int) TextEditor::ShortcutID::MoveBottom]      = TextEditor::Shortcut(ImGuiKey_End, 0, 1, 0);   // CTRL + END
        ret[(int) TextEditor::ShortcutID::SelectBottom]    = TextEditor::Shortcut(ImGuiKey_End, 0, 1, 1);   // CTRL + SHIFT + END
        ret[(int) TextEditor::ShortcutID::MoveStartLine]   = TextEditor::Shortcut(ImGuiKey_Home, 0, 0, 0);  // HOME
        ret[(int) TextEditor::ShortcutID::SelectStartLine] = TextEditor::Shortcut(ImGuiKey_Home, 0, 0, 1);  // SHIFT + HOME
        ret[(int) TextEditor::ShortcutID::MoveEndLine]     = TextEditor::Shortcut(ImGuiKey_End, 0, 0, 0);   // END
        ret[(int) TextEditor::ShortcutID::SelectEndLine]   = TextEditor::Shortcut(ImGuiKey_End, 0, 0, 1);   // SHIFT + END
        ret[(int) TextEditor::ShortcutID::ForwardDelete]   = TextEditor::Shortcut(ImGuiKey_Delete, 0, 0, 0);// DELETE
        ret[(int) TextEditor::ShortcutID::ForwardDeleteWord] = TextEditor::Shortcut(ImGuiKey_Delete, 0, 1, 0);// CTRL + DELETE
        ret[(int) TextEditor::ShortcutID::DeleteRight]       = TextEditor::Shortcut(ImGuiKey_Delete, 0, 0, 1);// SHIFT+BACKSPACE
        ret[(int) TextEditor::ShortcutID::BackwardDelete]    = TextEditor::Shortcut(ImGuiKey_Backspace, 0, 0, 0);// BACKSPACE
        ret[(int) TextEditor::ShortcutID::BackwardDeleteWord] =
                TextEditor::Shortcut(ImGuiKey_Backspace, 0, 1, 0);// CTRL + BACKSPACE
        ret[(int) TextEditor::ShortcutID::DeleteLeft]       = TextEditor::Shortcut(ImGuiKey_Backspace, 0, 0, 1);// SHIFT+BACKSPACE
        ret[(int) TextEditor::ShortcutID::OverwriteCursor]  = TextEditor::Shortcut(ImGuiKey_Insert, 0, 0, 0);   // INSERT
        ret[(int) TextEditor::ShortcutID::Copy]             = TextEditor::Shortcut(ImGuiKey_C, 0, 1, 0);        // CTRL+C
        ret[(int) TextEditor::ShortcutID::Paste]            = TextEditor::Shortcut(ImGuiKey_V, 0, 1, 0);        // CTRL+V
        ret[(int) TextEditor::ShortcutID::Cut]              = TextEditor::Shortcut(ImGuiKey_X, 0, 1, 0);        // CTRL+X
        ret[(int) TextEditor::ShortcutID::SelectAll]        = TextEditor::Shortcut(ImGuiKey_A, 0, 1, 0);        // CTRL+A
        ret[(int) TextEditor::ShortcutID::AutocompleteOpen] = TextEditor::Shortcut(ImGuiKey_Space, 0, 1, 0);    // CTRL+SPACE
        ret[(int) TextEditor::ShortcutID::AutocompleteSelect]       = TextEditor::Shortcut(ImGuiKey_Tab, 0, 0, 0);    // TAB
        ret[(int) TextEditor::ShortcutID::AutocompleteSelectActive] = TextEditor::Shortcut(ImGuiKey_Enter, 0, 0, 0);  // RETURN
        ret[(int) TextEditor::ShortcutID::AutocompleteUp]           = TextEditor::Shortcut(ImGuiKey_UpArrow, 0, 0, 0);// UP ARROW
        ret[(int) TextEditor::ShortcutID::AutocompleteDown] = TextEditor::Shortcut(ImGuiKey_DownArrow, 0, 0, 0);// DOWN ARROW
        ret[(int) TextEditor::ShortcutID::NewLine]          = TextEditor::Shortcut(ImGuiKey_Enter, 0, 0, 0);    // RETURN
        ret[(int) TextEditor::ShortcutID::Indent]           = TextEditor::Shortcut(ImGuiKey_Tab, 0, 0, 0);      // TAB
        ret[(int) TextEditor::ShortcutID::Unindent]         = TextEditor::Shortcut(ImGuiKey_Tab, 0, 0, 1);      // SHIFT + TAB
        ret[(int) TextEditor::ShortcutID::Find]             = TextEditor::Shortcut(ImGuiKey_F, 0, 1, 0);        // CTRL+F
        ret[(int) TextEditor::ShortcutID::Replace]          = TextEditor::Shortcut(ImGuiKey_Home, 0, 1, 0);     // CTRL+H
        ret[(int) TextEditor::ShortcutID::FindNext]         = TextEditor::Shortcut(ImGuiKey_F3, 0, 0, 0);       // F3
        ret[(int) TextEditor::ShortcutID::DebugStep]        = TextEditor::Shortcut(ImGuiKey_F10, 0, 0, 0);      // F10
        ret[(int) TextEditor::ShortcutID::DebugStepInto]    = TextEditor::Shortcut(ImGuiKey_F11, 0, 0, 0);      // F11
        ret[(int) TextEditor::ShortcutID::DebugStepOut]     = TextEditor::Shortcut(ImGuiKey_F11, 0, 0, 1);      // SHIFT+F11
        ret[(int) TextEditor::ShortcutID::DebugContinue]    = TextEditor::Shortcut(ImGuiKey_F5, 0, 0, 0);       // F5
        ret[(int) TextEditor::ShortcutID::DebugStop]        = TextEditor::Shortcut(ImGuiKey_F5, 0, 0, 1);       // SHIFT+F5
        ret[(int) TextEditor::ShortcutID::DebugBreakpoint]  = TextEditor::Shortcut(ImGuiKey_F9, 0, 0, 0);       // F9
        ret[(int) TextEditor::ShortcutID::DebugJumpHere]    = TextEditor::Shortcut(ImGuiKey_H, 1, 1, 0);        // CTRL+ALT+H
        ret[(int) TextEditor::ShortcutID::DuplicateLine]    = TextEditor::Shortcut(ImGuiKey_D, 0, 1, 0);        // CTRL+D
        ret[(int) TextEditor::ShortcutID::CommentLines]     = TextEditor::Shortcut(ImGuiKey_K, 0, 1, 1);        // CTRL+SHIFT+K
        ret[(int) TextEditor::ShortcutID::UncommentLines]   = TextEditor::Shortcut(ImGuiKey_U, 0, 1, 1);        // CTRL+SHIFT+U
    ...
    TextEditor::Line& TextEditor::InsertLine(int aIndex, int column)
    {
        ...
        auto btmp = mBreakpoints;
        mBreakpoints.clear();
        for (auto i : btmp) RemoveBreakpoint(i.mLine);
        for (auto i : btmp) AddBreakpoint(i.mLine >= aIndex ? i.mLine + 1 : i.mLine, i.mUseCondition, i.mCondition, i.mEnabled);
    ...
    TextEditor::Coordinates TextEditor::FindFirst(const std::string& what, const Coordinates& fromWhere)
    {
        ...
        while (loc != std::string::npos)
        {
            ...
            ret.mColumn = GetCharacterColumn(
                    ret.mLine, ret.mColumn);// ret.mColumn is currently character index, convert it to character column
    ...
    void TextEditor::HandleKeyboardInputs()
    {
        ...
        if (ImGui::IsWindowFocused())
        {
            ...
            for (int i = 0; i < m_shortcuts.size(); i++)
            {
                ...
                if (sct.Key == ImGuiKey_None)
                    continue;

                ShortcutID curActionID = ShortcutID::Count;
                bool additionalChecks  = true;

                if ((ImGui::IsKeyPressed(sct.Key) || (sct.Key == ImGuiKey_Enter && ImGui::IsKeyPressed(ImGuiKey_Enter))))
                {
            ...
            if (actionID != ShortcutID::Count)
            {
                ...
                switch (actionID)
                {
                    ...
                    case ShortcutID::BackwardDeleteWord:
                        if (ctrl &&
                            (mState.mSelectionStart == mState.mSelectionEnd || mState.mSelectionStart == mState.mCursorPosition))
                            MoveLeft(1, true, true);
                        Backspace();
                        break;
                    ...
                    case ShortcutID::Indent:
                        if (mIsSnippet)
                        {
                            ...
                            mSnippetTagLength = 0;
                            mSnippetTagPreviousLength =
                                    mSnippetTagEnd[mSnippetTagSelected].mColumn - mSnippetTagStart[mSnippetTagSelected].mColumn;
                    ...
                    case ShortcutID::DuplicateLine:
                    {
                        ...
                        undo.mAddedStart =
                                TextEditor::Coordinates(mState.mCursorPosition.mLine - 1, mState.mCursorPosition.mColumn);
            ...
            else if (!IsReadOnly())
            {
                for (int i = 0; i < io.InputQueueCharacters.Size; i++)
                {
                    ...
                    if (c != 0 && (c == '\n' || c >= 32))
                    {
                        ...
                        {
                            mSnippetTagLength++;
                            mSnippetTagEnd[mSnippetTagSelected].mColumn =
                                    mSnippetTagStart[mSnippetTagSelected].mColumn + mSnippetTagLength;
            ...
            {
                for (size_t i = 0; i < ImGuiKey_COUNT; i++) keyCount += ImGui::IsKeyPressed(static_cast<ImGuiKey>(i));
    ...
    void TextEditor::RenderInternal(const char* aTitle)
    {
        ...
        if (!mLines.empty())
        {
            ...
            if (mHighlightBrackets && mState.mSelectionStart == mState.mSelectionEnd)
            {
                if (mState.mCursorPosition.mLine <= mLines.size())
                {
                    ...
                    if (lineSize != 0 && start.mLine >= 0 && start.mLine < mLines.size() && start.mColumn >= 0 &&
                        start.mColumn < mLines[start.mLine].size())
                    {
            ...
            while (lineNo <= lineMax)
            {
                ImVec2 lineStartScreenPos =
                        ImVec2(cursorScreenPos.x, cursorScreenPos.y + (lineNo - linesFolded) * mCharAdvance.y);
                ...
                if (errorIt != mErrorMarkers.end())
                {
                    ...
                    if (ImGui::IsMouseHoveringRect(lineStartScreenPos, end))
                    {
                        ...
                        ImGui::PushStyleColor(ImGuiCol_Text,
                                              ImGui::ColorConvertU32ToFloat4(mPalette[(int) PaletteIndex::ErrorMessage]));
                ...
                for (int i = 0; i < line->size();)
                {
                    ...
                    if (lineFolded && lineFoldStartCIndex == i - 1)
                    {
                        ...
                        if (!mLineBuffer.empty())
                        {
                            ...
                            textSize = ImGui::GetFont()->CalcTextSizeA(ImGui::GetFontSize(), FLT_MAX, -1.0f, " ... ", nullptr,
                                                                       nullptr);
                            drawList->AddRect(ImVec2(textScreenPos.x + bufferOffset.x + mCharAdvance.x / 2.0f,
                                                     textScreenPos.y + bufferOffset.y),
                                              ImVec2(textScreenPos.x + bufferOffset.x + textSize.x - mCharAdvance.x / 2.0f,
                                                     textScreenPos.y + bufferOffset.y + mCharAdvance.y),
                ...
                if (mSidebar)
                {
                    drawList->AddRectFilled(
                            ...
                            ImVec2(lineStartScreenPos.x + scrollX + mTextStart - 5.0f, lineStartScreenPos.y + mCharAdvance.y),
                    ...
                    if (HasBreakpoint(lineNo + 1) != 0)
                    {
                        ...
                        if (!bkpt.mEnabled)
                            drawList->AddCircleFilled(ImVec2(startX, startY), radius - 1,
                                                      mPalette[(int) PaletteIndex::BreakpointDisabled]);
                        else
                        {
                            if (bkpt.mUseCondition)
                                drawList->AddRectFilled(ImVec2(startX - radius + 3, startY - radius / 4),
                                                        ImVec2(startX + radius - 3, startY + radius / 4),
                                                        mPalette[(int) PaletteIndex::BreakpointOutline]);
                        }
                    }
                    ...
                    if (lineNo + 1 == mDebugCurrentLine)
                    {
                        ...
                        // outline
                        drawList->AddRect(ImVec2(startX - radius, startY + radius / 2),
                                          ImVec2(startX, startY + radius * 3.0f / 2.0f),
                                          mPalette[(int) PaletteIndex::CurrentLineIndicatorOutline]);
                        drawList->AddTriangle(ImVec2(startX - 1, startY - 2), ImVec2(startX - 1, startY + radius * 2 + 1),
                                              ImVec2(startX + radius, startY + radius),
                                              mPalette[(int) PaletteIndex::CurrentLineIndicatorOutline]);

                        // fill
                        drawList->AddRectFilled(ImVec2(startX - radius + 1, startY + 1 + radius / 2),
                                                ImVec2(startX + 1, startY - 1 + radius * 3.0f / 2.0f),
                                                mPalette[(int) PaletteIndex::CurrentLineIndicator]);
                        drawList->AddTriangleFilled(ImVec2(startX, startY + 1), ImVec2(startX, startY - 1 + radius * 2),
                                                    ImVec2(startX - 1 + radius, startY + radius),
                                                    mPalette[(int) PaletteIndex::CurrentLineIndicator]);
                    }
                    ...
                    if (mShowLineNumbers)
                    {
                        ...
                        drawList->AddText(ImVec2(lineStartScreenPos.x + scrollX + mTextStart - lineNoWidth, lineStartScreenPos.y),
                    ...
                    if (mFoldEnabled)
                    {
                        ...
                        if (hasFold)
                        {
                            ...
                            // minus
                            drawList->AddLine(ImVec2(fmin.x + 3, (fmin.y + fmax.y) / 2.0f),
                                              ImVec2(fmax.x - 4, (fmin.y + fmax.y) / 2.0f),
                                              mPalette[(int) PaletteIndex::Default]);

                            // plus
                            if (isFolded)
                                drawList->AddLine(ImVec2((fmin.x + fmax.x) / 2.0f, fmin.y + 3),
                                                  ImVec2((fmin.x + fmax.x) / 2.0f, fmax.y - 4),
                                                  mPalette[(int) PaletteIndex::Default]);
                        ...
                        if (isHovered)
                        {
                            ...
                            drawList->AddRectFilled(pmin, pmax,
                                                    0x40000000 | (0x00FFFFFF & mPalette[(int) PaletteIndex::Default]));

                            // line bg
                            pmin.x = pmax.x;
                            pmax.x = lineStartScreenPos.x + scrollX + mTextStart + contentSize.x;
                            drawList->AddRectFilled(pmin, pmax,
                                                    0x20000000 | (0x00FFFFFF & mPalette[(int) PaletteIndex::Default]));
                        }
                    }
                }
            ...
            if (ImGui::IsMousePosValid() && (IsDebugging() || mFuncTooltips || ImGui::GetIO().KeyCtrl))
            {
                ...
                double hoverTime = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() -
                                                                                             mLastHoverTime)
                                           .count();
                ...
                else if (hoverTime > 0.2)
                {
                    ...
                    if (!id.empty())
                    {
                        ...
                        else
                        {
                            ...
                            if (!hasUnderline &&
                                mLanguageDefinition.mIdentifiers.find(id) != mLanguageDefinition.mIdentifiers.end())
                                hasUnderline = true;
                            ...
                            if (!hasUnderline)
                            {
                                ...
                                {
                                    if (wordCoords.mLine + 1 >= func.second.LineStart &&
                                        wordCoords.mLine + 1 <= func.second.LineEnd)
                                    {
                            ...
                            if (hasUnderline)
                            {
                                ImVec2 ulinePos = ImVec2(cursorScreenPos.x + mTextStart,
                                                         cursorScreenPos.y + wordStart.mLine * mCharAdvance.y);
                                drawList->AddLine(
                                        ImVec2(ulinePos.x + wordStart.mColumn * mCharAdvance.x, ulinePos.y + mCharAdvance.y),
        ...
        if (mFunctionDeclarationTooltip)
        {
            ...
            drawList->AddRectFilled(ttPos, ImVec2(ttPos.x + mUICalculateSize(ttWidth), ttPos.y + mUICalculateSize(ttHeight)),
        ...
        if (mACOpened)
        {
            ...
            drawList->AddRectFilled(acPos, ImVec2(acPos.x + mUICalculateSize(150), acPos.y + mUICalculateSize(100)),
                                    ImGui::GetColorU32(ImGuiCol_FrameBg));
        ...
        if (mFindOpened)
        {
            ImVec2 findPos = ImVec2(mUICursorPos.x + scrollX + mWindowWidth - mUICalculateSize(250),
                                    mUICursorPos.y + ImGui::GetScrollY() + mUICalculateSize(50) * (IsDebugging() && mDebugBar));
            drawList->AddRectFilled(
                    findPos, ImVec2(findPos.x + mUICalculateSize(220), findPos.y + mUICalculateSize(mReplaceOpened ? 90 : 40)),
                    ImGui::GetColorU32(ImGuiCol_WindowBg));
        }
        if (IsDebugging() && mDebugBar)
        {
            ImVec2 dbgPos = ImVec2(mUICursorPos.x + scrollX + mWindowWidth / 2 - mDebugBarWidth / 2,
                                   mUICursorPos.y + ImGui::GetScrollY());
            drawList->AddRectFilled(dbgPos, ImVec2(dbgPos.x + mDebugBarWidth, dbgPos.y + mDebugBarHeight),
                                    ImGui::GetColorU32(ImGuiCol_FrameBg));
        }
    }
    ...
    void TextEditor::m_buildMemberSuggestions(bool* keepACOpened)
    {
        ...
        for (auto& func : mACFunctions)
        {
            ...
            if (mState.mCursorPosition.mLine >= func.second.LineStart - 2 &&
                mState.mCursorPosition.mLine <= func.second.LineEnd + 1)
            {
                // locals
    ...
    void TextEditor::m_buildSuggestions(bool* keepACOpened)
    {
        ...
        if (isValid)
        {
            ...
            {
                // get the words
                for (size_t i = 0; i < mACEntrySearch.size(); i++)
                {
    ...
    void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
    {
        ...
        if (!mIgnoreImGuiChild)
            ImGui::BeginChild(aTitle, aSize, aBorder,
                              (ImGuiWindowFlags_AlwaysHorizontalScrollbar * mHorizontalScroll) | ImGuiWindowFlags_NoMove |
        ...
        if (mScrollbarMarkers)
        {
            ...
            if (window->ScrollbarY)
            {
                ...
                {
                    float lineStartY =
                            std::round(scrollBarRect.Min.y + (mSelectedLine - 0.5f) / mLines.size() * scrollBarRect.GetHeight());
                    drawList->AddLine(ImVec2(scrollBarRect.Min.x, lineStartY), ImVec2(scrollBarRect.Max.x, lineStartY),
                                      (mPalette[(int) PaletteIndex::Default] & 0x00FFFFFFu) | 0x83000000u, 3);
                }
                ...
                {
                    float lineStartY =
                            std::round(scrollBarRect.Min.y + (float(line) - 0.5f) / mLines.size() * scrollBarRect.GetHeight());
                    float lineEndY = std::round(scrollBarRect.Min.y +
                                                (float(line + 1) - 0.5f) / mLines.size() * scrollBarRect.GetHeight());
                    drawList->AddRectFilled(ImVec2(scrollBarRect.Min.x + scrollBarRect.GetWidth() * 0.6f, lineStartY),
                ...
                for (auto& error : mErrorMarkers)
                {
                    float lineStartY = std::round(scrollBarRect.Min.y +
                                                  (float(error.first) - 0.5f) / mLines.size() * scrollBarRect.GetHeight());
                    drawList->AddRectFilled(ImVec2(scrollBarRect.Min.x, lineStartY),
                                            ImVec2(scrollBarRect.Min.x + scrollBarRect.GetWidth() * 0.4f, lineStartY + 6.0f),
        ...
        if (mFindOpened)
        {
            ImGui::SetNextWindowPos(ImVec2(mFindOrigin.x + windowWidth - mUICalculateSize(250),
                                           mFindOrigin.y + mUICalculateSize(50) * (IsDebugging() && mDebugBar)),
                                    ImGuiCond_Always);
            ImGui::BeginChild(("##ted_findwnd" + std::string(aTitle)).c_str(),
                              ImVec2(mUICalculateSize(220), mUICalculateSize(mReplaceOpened ? 90 : 40)), true,
            ...
            if (ImGui::InputText(("##ted_findtextbox" + std::string(aTitle)).c_str(), mFindWord, 256,
                                 ImGuiInputTextFlags_EnterReturnsTrue) ||
                mFindNext)
            {
                auto curPos   = mState.mCursorPosition;
                size_t cindex = 0;
                for (size_t ln = 0; ln < curPos.mLine; ln++) cindex += GetLineCharacterCount(ln) + 1;
            ...
            if (mReplaceOpened && !mReadOnly)
            {
                ...
                if (ImGui::InputText(("##ted_replacetb" + std::string(aTitle)).c_str(), mReplaceWord, 256,
                                     ImGuiInputTextFlags_EnterReturnsTrue))
                    shouldReplace = true;
                if (ImGui::IsItemActive())
        ...
        if (IsDebugging() && mDebugBar)
        {
            ImGui::SetNextWindowPos(ImVec2(mFindOrigin.x + windowWidth / 2 - mDebugBarWidth / 2, mFindOrigin.y),
                                    ImGuiCond_Always);
            ImGui::BeginChild(("##ted_dbgcontrols" + std::string(aTitle)).c_str(), ImVec2(mDebugBarWidth, mDebugBarHeight), true,
                              ImGuiWindowFlags_NoScrollbar);
    ...
    void TextEditor::EnterCharacter(ImWchar aChar, bool aShift)
    {
        ...
        if (aChar == '\n')
        {
            ...
            int foldOffset = 0;
            for (int i = 0; i < cindex; i++) foldOffset -= 1 + (line[i].mChar == '\t') * 3;
            ...
            for (int b = 0; b < mFoldBegin.size(); b++)
                if (mFoldBegin[b].mLine == coord.mLine + 1)
                    mFoldBegin[b].mColumn =
                            std::max<int>(0, (mFoldBegin[b].mColumn + foldOffset) + (mFoldBegin[b].mColumn != coord.mColumn));
            for (int b = 0; b < mFoldEnd.size(); b++)
                if (mFoldEnd[b].mLine == coord.mLine + 1)
                    mFoldEnd[b].mColumn =
                            std::max<int>(0, (mFoldEnd[b].mColumn + foldOffset) + (mFoldEnd[b].mColumn != coord.mColumn));
        }
        else
        {
            char buf[7];
            ...
            if (e > 0)
            {
                ...
                {
                    for (int i = 0; i < mTabSize; i++) buf[i] = ' ';
                    e = mTabSize;
                }
                buf[e] = '\0';
    ...
    void TextEditor::SetSelection(const Coordinates& aStart, const Coordinates& aEnd, SelectionMode aMode)
    {
        ...
        mReplaceIndex = 0;
        for (size_t ln = 0; ln < mState.mCursorPosition.mLine; ln++) mReplaceIndex += GetLineCharacterCount(ln) + 1;
    ...
    void TextEditor::MoveDown(int aAmount, bool aSelect)
    {
        ...
        auto oldPos = mState.mCursorPosition;
        mState.mCursorPosition.mLine =
                std::max<int>(0, std::min<int>((int) mLines.size() - 1, mState.mCursorPosition.mLine + aAmount));
    ...
    void TextEditor::Backspace()
    {
        ...
        else
        {
            ...
            if (mState.mCursorPosition.mColumn == 0)
            {
                ...
                ErrorMarkers etmp;
                for (auto& i : mErrorMarkers)
                    etmp.insert(ErrorMarkers::value_type(i.first - 1 == mState.mCursorPosition.mLine ? i.first - 1 : i.first,
                                                         i.second));
                mErrorMarkers = std::move(etmp);
    ...
    std::vector<std::string> TextEditor::GetRelevantExpressions(int line)
    {
        ...
        std::string expr = "";
        for (int i = 0; i < mLines[line].size(); i++) expr += mLines[line][i].mChar;
        ...
        for (int i = 0; i < tokens.size(); i++)
        {
            if ((forSection == -1 || forSection == 1) && tokens[i].Type == TokenType::Identifier && i + 1 < tokens.size() &&
                tokens[i + 1].Content == "(")
            {
            ...
            else if ((forSection == -1 || forSection == 1) && (tokens[i].Type == TokenType::Comma || tokens[i].Content == ")") &&
    ...
    const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::HLSL()
    {
        ...
        if (!inited)
        {
            static const char* const keywords[] = {"AppendStructuredBuffer",
                                                   "asm",
                                                   "asm_fragment",
                                                   "BlendState",
                                                   "bool",
    ...
    void TextEditor::LanguageDefinition::m_HLSLDocumentation(Identifiers& idents)
    {
        ...
        idents.insert(std::make_pair(
                "AllMemoryBarrier",
                Identifier("Blocks execution of all threads in a group until all memory accesses have been completed.")));
        idents.insert(std::make_pair("AllMemoryBarrierWithGroupSync",
                                     Identifier("Blocks execution of all threads in a group until all memory accesses have been "
                                                "completed and all threads in the group have reached this call.")));
        ...
        idents.insert(
                std::make_pair("CheckAccessFullyMapped", Identifier("Determines whether all values from a Sample or Load "
        ...
        idents.insert(
                std::make_pair("D3DCOLORtoUBYTE4", Identifier("Swizzles and scales components of the 4D vector x to compensate "
                                                              "for the lack of UBYTE4 support in some hardware.")));
        idents.insert(std::make_pair(
                "ddx", Identifier("Returns the partial derivative of x with respect to the screen-space x-coordinate.")));
        idents.insert(std::make_pair(
                "ddx_coarse",
                Identifier("Computes a low precision partial derivative with respect to the screen-space x-coordinate.")));
        idents.insert(std::make_pair(
                "ddx_fine",
                Identifier("Computes a high precision partial derivative with respect to the screen-space x-coordinate.")));
        idents.insert(std::make_pair(
                "ddy", Identifier("Returns the partial derivative of x with respect to the screen-space y-coordinate.")));
        idents.insert(std::make_pair(
                "ddy_coarse", Identifier("Returns the partial derivative of x with respect to the screen-space y-coordinate.")));
        idents.insert(std::make_pair(
                "ddy_fine",
                Identifier("Computes a high precision partial derivative with respect to the screen-space y-coordinate.")));
        ...
        idents.insert(std::make_pair(
                "DeviceMemoryBarrier",
                Identifier("Blocks execution of all threads in a group until all device memory accesses have been completed.")));
        idents.insert(std::make_pair("DeviceMemoryBarrierWithGroupSync",
                                     Identifier("Blocks execution of all threads in a group until all device memory accesses "
        ...
        idents.insert(
                std::make_pair("f16tof32", Identifier("Converts the float16 stored in the low-half of the uint to a float.")));
        ...
        idents.insert(std::make_pair("firstbithigh", Identifier("Gets the location of the first set bit starting from the "
                                                                "highest order bit and working downward, per component.")));
        idents.insert(std::make_pair("firstbitlow", Identifier("Returns the location of the first set bit starting from the "
                                                               "lowest order bit and working upward, per component.")));
        ...
        idents.insert(std::make_pair("GetRenderTargetSamplePosition",
                                     Identifier("Returns a sample position (x,y) for a given sample index.")));
        idents.insert(std::make_pair(
                "GroupMemoryBarrier",
                Identifier("Blocks execution of all threads in a group until all group shared accesses have been completed.")));
        idents.insert(std::make_pair("GroupMemoryBarrierWithGroupSync",
                                     Identifier("Blocks execution of all threads in a group until all group shared accesses have "
                                                "been completed and all threads in the group have reached this call.")));
        idents.insert(std::make_pair("InterlockedAdd",
                                     Identifier("Performs a guaranteed atomic add of value to the dest resource variable.")));
        idents.insert(std::make_pair("InterlockedAnd", Identifier("Performs a guaranteed atomic and.")));
        idents.insert(
                std::make_pair("InterlockedCompareExchange",
                               Identifier("Atomically compares the input to the comparison value and exchanges the result.")));
        idents.insert(
                std::make_pair("InterlockedCompareStore", Identifier("Atomically compares the input to the comparison value.")));
        ...
        idents.insert(std::make_pair(
                "msad4",
                Identifier("Compares a 4-byte reference value and an 8-byte source value and accumulates a vector of 4 sums.")));
        ...
        idents.insert(std::make_pair("Process2DQuadTessFactorsAvg",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("Process2DQuadTessFactorsMax",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("Process2DQuadTessFactorsMin",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("ProcessIsolineTessFactors",
                                     Identifier("Generates the rounded tessellation factors for an isoline.")));
        idents.insert(std::make_pair("ProcessQuadTessFactorsAvg",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("ProcessQuadTessFactorsMax",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("ProcessQuadTessFactorsMin",
                                     Identifier("Generates the corrected tessellation factors for a quad patch.")));
        idents.insert(std::make_pair("ProcessTriTessFactorsAvg",
                                     Identifier("Generates the corrected tessellation factors for a tri patch.")));
        idents.insert(std::make_pair("ProcessTriTessFactorsMax",
                                     Identifier("Generates the corrected tessellation factors for a tri patch.")));
        idents.insert(std::make_pair("ProcessTriTessFactorsMin",
                                     Identifier("Generates the corrected tessellation factors for a tri patch.")));
    ...
    void TextEditor::LanguageDefinition::m_GLSLDocumentation(Identifiers& idents)
    {
        ...
        idents.insert(
                std::make_pair("radians", Identifier("genType radians(genType degrees)\nConverts x from degrees to radians.")));
        idents.insert(
                std::make_pair("degrees", Identifier("genType degrees(genType radians)\nConverts x from radians to degrees.")));
        ...
        idents.insert(
                std::make_pair("acos", Identifier("genType acos(genType x)\nReturns the arccosine of each component of x.")));
        idents.insert(std::make_pair(
                "atan",
                Identifier("genType atan(genType y, genType x)\ngenType atan(genType y_over_x)\nReturns the arctangent of x.")));
        ...
        idents.insert(std::make_pair(
                "sqrt", Identifier("genType sqrt(genType x)\ngenDType sqrt(genDType x)\nSquare root (per component).")));
        idents.insert(std::make_pair(
                "inversesqrt",
                Identifier("genType inversesqrt(genType x)\ngenDType inversesqrt(genDType x)\nReturns rcp(sqrt(x)).")));
        ...
        idents.insert(std::make_pair("trunc", Identifier("genType trunc(genType x)\ngenDType trunc(genDType x)\nTruncates "
                                                         "floating-point value(s) to integer value(s)")));
        idents.insert(std::make_pair(
                "round", Identifier("genType round(genType x)\ngenDType round(genDType x)\nRounds x to the nearest integer")));
        idents.insert(std::make_pair(
                "roundEven",
                Identifier("genType roundEven(genType x)\ngenDType roundEven(genDType x)\nReturns a value equal to the nearest "
                           "integer to x. A fractional part of 0.5 will round toward the nearest even integer.")));
        ...
        idents.insert(std::make_pair("clamp",
                                     Identifier("genType clamp(genType x, genType minVal, genType maxVal)\ngenType clamp(genType "
                                                "x, float minVal, float maxVal)\nClamps x to the range [min, max].")));
        ...
        idents.insert(std::make_pair("isinf", Identifier("genBType isinf(genType x)\ngenBType isinf(genDType x)\nReturns true if "
                                                         "x is +INF or -INF, false otherwise.")));
        idents.insert(std::make_pair("isnan", Identifier("genBType isnan(genType x)\ngenBType isnan(genDType x)\nReturns true if "
                                                         "x is NAN or QNAN, false otherwise.")));
        idents.insert(std::make_pair(
                "smoothstep",
                Identifier("genType smoothstep(genType edge0, genType edge1, genType x)\ngenType smoothstep(float edge0, float "
        ...
        idents.insert(std::make_pair(
                "floatBitsToInt",
                Identifier(
                        "genIType floatBitsToInt(genType x)\nReturns a signed or unsigned integer value representing the "
                        "encoding of a floating-point value. The floatingpoint value's bit-level representation is preserved.")));
        idents.insert(std::make_pair(
                "floatBitsToUint",
                Identifier(
                        ...
                        "encoding of a floating-point value. The floatingpoint value's bit-level representation is preserved.")));
        idents.insert(std::make_pair(
                "intBitsToFloat", Identifier("genType intBitsToFloat(genIType x)\nReturns a floating-point value corresponding "
                                             "to a signed or unsigned integer encoding of a floating-point value.")));
        idents.insert(std::make_pair(
                "uintBitsToFloat", Identifier("genType uintBitsToFloat(genUType x)\nReturns a floating-point value corresponding "
                                              "to a signed or unsigned integer encoding of a floating-point value.")));
        idents.insert(std::make_pair("fmod", Identifier("Returns the floating point remainder of x/y.")));
        idents.insert(std::make_pair("fma", Identifier("genType fma(genType a, genType b, genType c)\nReturns the "
                                                       "double-precision fused multiply-addition of a * b + c.")));
        idents.insert(std::make_pair("ldexp", Identifier("genType ldexp(genType x, genIType exp)\nReturns x * 2exp")));
        idents.insert(std::make_pair("packUnorm2x16",
                                     Identifier("uint packUnorm2x16(vec2 v)\nFirst, converts each component of the normalized "
                                                "floating - point value v into 8 or 16bit integer values. Then, the results are "
                                                "packed into the returned 32bit unsigned integer.")));
        idents.insert(std::make_pair("packUnorm4x8",
                                     Identifier("uint packUnorm4x8(vec4 v)\nFirst, converts each component of the normalized "
                                                "floating - point value v into 8 or 16bit integer values. Then, the results are "
                                                "packed into the returned 32bit unsigned integer.")));
        idents.insert(std::make_pair("packSnorm4x8",
                                     Identifier("uint packUnorm4x8(vec4 v)\nFirst, converts each component of the normalized "
                                                "floating - point value v into 8 or 16bit integer values. Then, the results are "
                                                "packed into the returned 32bit unsigned integer.")));
        idents.insert(std::make_pair("unpackUnorm2x16",
                                     Identifier("vec2 unpackUnorm2x16(uint p)\nFirst, unpacks a single 32bit unsigned integer p "
                                                "into a pair of 16bit unsigned integers, four 8bit unsigned integers, or four "
                                                "8bit signed integers.Then, each component is converted to a normalized floating "
                                                "point value to generate the returned two or four component vector.")));
        idents.insert(std::make_pair("unpackUnorm4x8",
                                     Identifier("vec4 unpackUnorm4x8(uint p)\nFirst, unpacks a single 32bit unsigned integer p "
                                                "into a pair of 16bit unsigned integers, four 8bit unsigned integers, or four "
                                                "8bit signed integers.Then, each component is converted to a normalized floating "
                                                "point value to generate the returned two or four component vector.")));
        idents.insert(std::make_pair("unpackSnorm4x8",
                                     Identifier("vec4 unpackSnorm4x8(uint p)\nFirst, unpacks a single 32bit unsigned integer p "
                                                "into a pair of 16bit unsigned integers, four 8bit unsigned integers, or four "
                                                "8bit signed integers.Then, each component is converted to a normalized floating "
                                                "point value to generate the returned two or four component vector.")));
        idents.insert(
                std::make_pair("packDouble2x32", Identifier("double packDouble2x32(uvec2 v)\nReturns a double-precision value "
                                                            "obtained by packing the components of v into a 64-bit value.")));
        idents.insert(std::make_pair("unpackDouble2x32", Identifier("uvec2 unpackDouble2x32(double d)\nReturns a two-component "
                                                                    "unsigned integer vector representation of v.")));
        idents.insert(std::make_pair("length", Identifier("float length(genType x)\nReturns the length of the vector v.")));
        idents.insert(std::make_pair(
                "distance", Identifier("float distance(genType p0, genType p1)\nReturns the distance between two points.")));
        idents.insert(
                std::make_pair("dot", Identifier("float dot(genType x, genType y)\nReturns the dot product of two vectors.")));
        idents.insert(
                std::make_pair("cross", Identifier("vec3 cross(vec3 x, vec3 y)\nReturns the cross product of two 3D vectors.")));
        idents.insert(std::make_pair("normalize", Identifier("genType normalize(genType v)\nReturns a normalized vector.")));
        idents.insert(std::make_pair(
                "faceforward",
                Identifier("genType faceforward(genType N, genType I, genType Nref)\nReturns -n * sign(dot(i, ng)).")));
        idents.insert(
                std::make_pair("reflect", Identifier("genType reflect(genType I, genType N)\nReturns a reflection vector.")));
        idents.insert(std::make_pair(
                "refract", Identifier("genType refract(genType I, genType N, float eta)\nReturns the refraction vector.")));
        idents.insert(std::make_pair(
                "matrixCompMult", Identifier("mat matrixCompMult(mat x, mat y)\nMultiply matrix x by matrix y component-wise.")));
        ...
        idents.insert(std::make_pair("determinant",
                                     Identifier("float determinant(mat m)\nReturns the determinant of the square matrix m.")));
        idents.insert(std::make_pair("inverse", Identifier("mat inverse(mat m)\nReturns a matrix that is the inverse of m.")));
        idents.insert(std::make_pair("lessThan",
                                     Identifier("bvec lessThan(vec x, vec y)\nReturns the component-wise compare of x < y")));
        idents.insert(std::make_pair(
                "lessThanEqual", Identifier("bvec lessThanEqual(vec x, vec y)\nReturns the component-wise compare of x <= y")));
        idents.insert(std::make_pair("greaterThan",
                                     Identifier("bvec greaterThan(vec x, vec y)\nReturns the component-wise compare of x > y")));
        idents.insert(
                std::make_pair("greaterThanEqual",
                               Identifier("bvec greaterThanEqual(vec x, vec y)\nReturns the component-wise compare of x >= y")));
        idents.insert(
                std::make_pair("equal", Identifier("bvec equal(vec x, vec y)\nReturns the component-wise compare of x == y")));
        idents.insert(std::make_pair("notEqual",
                                     Identifier("bvec notEqual(vec x, vec y)\nReturns the component-wise compare of x != y")));
        ...
        idents.insert(
                std::make_pair("uaddCarry", Identifier("genUType uaddCarry(genUType x, genUType y, out genUType carry)\nAdds "
                                                       "32bit unsigned integer x and y, returning the sum modulo 2^32.")));
        idents.insert(std::make_pair(
                "usubBorrow",
                Identifier(
                        "genUType usubBorrow(genUType x, genUType y, out genUType borrow)\nSubtracts the 32bit unsigned integer "
                        "y from x, returning the difference if non-negatice, or 2^32 plus the difference otherwise.")));
        idents.insert(std::make_pair("umulExtended",
                                     Identifier("void umulExtended(genUType x, genUType y, out genUType msb, out genUType "
                                                "lsb)\nMultiplies 32bit integers x and y, producing a 64bit result.")));
        idents.insert(std::make_pair("imulExtended",
                                     Identifier("void imulExtended(genIType x, genIType y, out genIType msb, out genIType "
                                                "lsb)\nMultiplies 32bit integers x and y, producing a 64bit result.")));
        idents.insert(std::make_pair(
                "bitfieldExtract",
                Identifier("genIType bitfieldExtract(genIType value, int offset, int bits)\ngenUType bitfieldExtract(genUType "
                           "value, int offset, int bits)\nExtracts bits [offset, offset + bits - 1] from value, returning them "
                           "in the least significant bits of the result.")));
        idents.insert(std::make_pair(
                "bitfieldInsert", Identifier("genIType bitfieldInsert(genIType base, genIType insert, int offset, int "
                                             "bits)\ngenUType bitfieldInsert(genUType base, genUType insert, int offset, int "
                                             "bits)\nReturns the insertion the bits leas-significant bits of insert into base")));
        idents.insert(std::make_pair("bitfieldReverse",
                                     Identifier("genIType bitfieldReverse(genIType value)\ngenUType bitfieldReverse(genUType "
                                                "value)\nReturns the reversal of the bits of value.")));
        idents.insert(std::make_pair("bitCount",
                                     Identifier("genIType bitCount(genIType value)\ngenUType bitCount(genUType value)\nReturns "
                                                "the number of bits set to 1 in the binary representation of value.")));
        idents.insert(std::make_pair(
                "findLSB", Identifier("genIType findLSB(genIType value)\ngenUType findLSB(genUType value)\nReturns the bit "
                                      "number of the least significant bit set to 1 in the binary representation of value.")));
        idents.insert(std::make_pair(
                "findMSB", Identifier("genIType findMSB(genIType value)\ngenUType findMSB(genUType value)\nReturns the bit "
                                      "number of the most significant bit in the binary representation of value.")));
        idents.insert(
                std::make_pair("textureSize", Identifier("ivecX textureSize(gsamplerXD sampler, int lod)\nReturns the dimensions "
                                                         "of level lod  (if present) for the texture bound to sample.")));
        idents.insert(std::make_pair("textureQueryLod",
                                     Identifier("vec2 textureQueryLod(gsamplerXD sampler, vecX P)\nReturns the mipmap array(s) "
                                                "that would be accessed in the x component of the return value.")));
        idents.insert(std::make_pair(
                "texture", Identifier("gvec4 texture(gsamplerXD sampler, vecX P, [float bias])\nUse the texture coordinate P to "
                                      "do a texture lookup in the texture currently bound to sampler.")));
        ...
        idents.insert(std::make_pair(
                "textureOffset",
                Identifier("gvec4 textureOffset(gsamplerXD sampler, vecX P, ivecX offset, [float bias])\nDo a texture lookup as "
                           "in texture but with offset added to the (u,v,w) texel coordinates before looking up each texel.")));
        idents.insert(
                std::make_pair("texelFetch", Identifier("gvec4 texelFetch(gsamplerXD sampler, ivecX P, int lod)\nUse integer "
                                                        "texture coordinate P to lookup a single texel from sampler.")));
        idents.insert(std::make_pair("texelFetchOffset",
                                     Identifier("gvec4 texelFetchOffset(gsamplerXD sampler, ivecX P, int lod, int offset)\nFetch "
                                                "a single texel as in texelFetch offset by offset.")));
        idents.insert(std::make_pair("textureProjLod", Identifier("Do a projective texture lookup with explicit LOD.")));
        idents.insert(
                std::make_pair("textureLodOffset", Identifier("gvec4 textureLodOffset(gsamplerXD sampler, vecX P, float lod, "
                                                              "ivecX offset)\nDo an offset texture lookup with explicit LOD.")));
        idents.insert(
                std::make_pair("textureProjLodOffset", Identifier("Do an offset projective texture lookup with explicit LOD.")));
        idents.insert(std::make_pair("textureGrad",
                                     Identifier("gvec4 textureGrad(gsamplerXD sampler, vecX P, vecX dPdx, vecX dPdy)\nDo a "
                                                "texture lookup as in texture but with explicit gradients.")));
        idents.insert(std::make_pair(
                "textureGradOffset",
                Identifier(
                        "gvec4 textureGradOffset(gsamplerXD sampler, vecX P, vecX dPdx, vecX dPdy, ivecX offset)\nDo a texture "
                        "lookup with both explicit gradient and offset, as described in textureGrad and textureOffset.")));
        idents.insert(std::make_pair("textureProjGrad",
                                     Identifier("Do a texture lookup both projectively and with explicit gradient.")));
        idents.insert(std::make_pair(
                "textureProjGradOffset",
                Identifier("Do a texture lookup both projectively and with explicit gradient as well as with offset.")));
        idents.insert(std::make_pair(
                "textureGather",
                Identifier("gvec4 textureGather(gsampler2D sampler, vec2 P, [int comp])\nGathers four texels from a texture")));
        idents.insert(std::make_pair("textureGatherOffset",
                                     Identifier("gvec4 textureGatherOffset(gsampler2D sampler, vec2 P, ivec2 offset, [int "
                                                "comp])\nGathers four texels from a texture with offset.")));
        idents.insert(std::make_pair("textureGatherOffsets",
                                     Identifier("gvec4 textureGatherOffsets(gsampler2D sampler, vec2 P, ivec2 offsets[4], [int "
        ...
        idents.insert(std::make_pair("dFdx", Identifier("genType dFdx(genType p)\nReturns the partial derivative of x with "
                                                        "respect to the screen-space x-coordinate.")));
        idents.insert(std::make_pair("dFdy", Identifier("genType dFdy(genType p)\nReturns the partial derivative of x with "
                                                        "respect to the screen-space y-coordinate.")));
        idents.insert(std::make_pair("fwidth", Identifier("genType fwidth(genType p)\nReturns abs(ddx(x)) + abs(ddy(x))")));
        idents.insert(std::make_pair("interpolateAtCentroid",
                                     Identifier("Return the value of the input varying interpolant sampled at a location inside "
                                                "the both the pixel and the primitive being processed.")));
        idents.insert(std::make_pair(
                "interpolateAtSample",
                Identifier("Return the value of the input varying interpolant at the location of sample number sample.")));
        idents.insert(
                std::make_pair("interpolateAtOffset", Identifier("Return the value of the input varying interpolant sampled at "
                                                                 "an offset from the center of the pixel specified by offset.")));
        ...
        idents.insert(std::make_pair("EmitStreamVertex",
                                     Identifier("void EmitStreamVertex(int stream)\nEmit the current values of output variables "
                                                "to the current output primitive on stream stream.")));
        idents.insert(
                std::make_pair("EndStreamPrimitive", Identifier("void EndStreamPrimitive(int stream)\nCompletes the current "
                                                                "output primitive on stream stream and starts a new one.")));
        idents.insert(std::make_pair("EmitVertex",
                                     Identifier("void EmitVertex()\nEmit the current values to the current output primitive.")));
        idents.insert(std::make_pair(
                "EndPrimitive", Identifier("void EndPrimitive()\nCompletes the current output primitive and starts a new one.")));
        idents.insert(
                std::make_pair("barrier", Identifier("void barrier()\nSynchronize execution of multiple shader invocations")));
        idents.insert(std::make_pair("groupMemoryBarrier",
                                     Identifier("void groupMemoryBarrier()\nControls the ordering of memory transaction issued "
                                                "shader invocation relative to a work group")));
        idents.insert(std::make_pair("memoryBarrier", Identifier("uint memoryBarrier()\nControls the ordering of memory "
                                                                 "transactions issued by a single shader invocation")));
        idents.insert(std::make_pair("memoryBarrierAtomicCounter",
                                     Identifier("void memoryBarrierAtomicCounter()\nControls the ordering of operations on "
                                                "atomic counters issued by a single shader invocation")));
        idents.insert(std::make_pair("memoryBarrierBuffer",
                                     Identifier("void memoryBarrierBuffer()\nControls the ordering of operations on buffer "
                                                "variables issued by a single shader invocation")));
        idents.insert(
                std::make_pair("memoryBarrierImage", Identifier("void memoryBarrierImage()\nControls the ordering of operations "
                                                                "on image variables issued by a single shader invocation")));
        idents.insert(std::make_pair("memoryBarrierShared",
                                     Identifier("void memoryBarrierShared()\nControls the ordering of operations on shared "
                                                "variables issued by a single shader invocation")));

        idents.insert(std::make_pair("atomicAdd", Identifier("int atomicAdd(inout int mem, int data)\nuint atomicAdd(inout uint "
                                                             "mem, uint data)\nPerform an atomic addition to a variable")));
        idents.insert(
                std::make_pair("atomicAnd", Identifier("int atomicAnd(inout int mem, int data)\nuint atomicAnd(inout uint mem, "
                                                       "uint data)\nPerform an atomic logical AND operation to a variable")));
        idents.insert(std::make_pair(
                "atomicCompSwap",
                Identifier("int atomicCompSwap(inout int mem, uint compare, uint data)\nuint atomicCompSwap(inout uint mem, uint "
                           "compare, uint data)\nPerform an atomic compare-exchange operation to a variable")));
        idents.insert(std::make_pair(
                "atomicCounter", Identifier("uint atomicCounter(atomic_uint c)\nReturn the current value of an atomic counter")));
        idents.insert(std::make_pair(
                "atomicCounterDecrement",
                Identifier(
                        "uint atomicCounterDecrement(atomic_uint c)\nAtomically decrement a counter and return its new value")));
        idents.insert(
                std::make_pair("atomicCounterIncrement", Identifier("uint atomicCounterIncrement(atomic_uint c)\nAtomically "
                                                                    "increment a counter and return the prior value")));
        idents.insert(std::make_pair("atomicExchange",
                                     Identifier("int atomicExchange(inout int mem, int data)\nuint atomicExchange(inout uint "
                                                "mem, uint data)\nPerform an atomic exchange operation to a variable ")));
        idents.insert(std::make_pair("atomicMax", Identifier("int atomicMax(inout int mem, int data)\nuint atomicMax(inout uint "
                                                             "mem, uint data)\nPerform an atomic max operation to a variable")));
        idents.insert(std::make_pair("atomicMin", Identifier("int atomicMin(inout int mem, int data)\nuint atomicMin(inout uint "
                                                             "mem, uint data)\nPerform an atomic min operation to a variable ")));
        idents.insert(
                std::make_pair("atomicOr", Identifier("int atomicOr(inout int mem, int data)\nuint atomicOr(inout uint mem, uint "
                                                      "data)\nPerform an atomic logical OR operation to a variable")));
        idents.insert(std::make_pair("atomicXor",
                                     Identifier("int atomicXor(inout int mem, int data)\nuint atomicXor(inout uint mem, uint "
