...
namespace Engine::EditorPipelines
{
	...
	{
		m_scene_color   = find_param_info("args.scene_color");
		m_scene_depth   = find_param_info("args.scene_depth");
		m_outline_depth = find_param_info("args.outline_depth");
		m_linear        = find_param_info("args.linear");
		m_point         = find_param_info("args.point");
		m_outline_color = find_param_info("args.outline_color");
		m_near          = find_param_info("args.near");
		m_far           = find_param_info("args.far");
		m_sample_offset = find_param_info("args.sample_offset");

		SamplerInitializer initializer;
		initializer.address_u = SamplerAddressMode::ClampToEdge;
		initializer.address_v = SamplerAddressMode::ClampToEdge;
		initializer.address_w = SamplerAddressMode::ClampToEdge;
		initializer.filter    = SamplerFilter::Point;

		m_point_sampler.init(initializer);

		initializer.filter = SamplerFilter::Bilinear;
	...
	void Outline::render(Renderer* renderer, SRV* outline_depth, Vector3f color, Vector2f sample_offset)
	{
		...
		Rect2D rect({}, view_size);
		tmp_color->as_rtv()->blit(renderer->scene_color_target()->as_rtv(), rect, rect, SamplerFilter::Point);
		...
		rhi_bind();
		rhi->bind_srv(scene_color, m_scene_color->location);
		rhi->bind_srv(scene_depth, m_scene_depth->location);
		rhi->bind_srv(outline_depth, m_outline_depth->location);

		m_point_sampler.rhi_sampler()->bind(m_point->location);
		m_linear_sampler.rhi_sampler()->bind(m_linear->location);
