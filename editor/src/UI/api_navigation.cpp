...
namespace Trinex::UI
{
	...
	bool begin_collapsing_header(StringView label, const HeaderOptions& options)
	{
		...
		if (animated_row(label, options.icon, options.right_text, open, options.disabled, ImVec2(0, imgui_frame_height()),
		                 options.accent, true, anim.open))
		{
			open = !open;
	...
	bool tree_node(StringView label, const TreeNodeOptions& options)
	{
		...
		const bool clicked = animated_row(label, options.icon, options.badge, options.selected, false,
		                                  ImVec2(ImGui::GetContentRegionAvail().x, imgui_frame_height()), options.accent,
		                                  !options.leaf, anim.open);
	...
	bool tree_leaf(StringView label, const TreeNodeOptions& options)
	{
		...
		return animated_row(label, copy.icon, copy.badge, copy.selected, false,
		                    ImVec2(ImGui::GetContentRegionAvail().x, imgui_frame_height()), copy.accent, false, 0.0f);
	}
	...
	bool begin_menu(StringView label, bool enabled)
	{
		...
		ImGui::SetNextWindowBgAlpha(panel_color().w * ImGui::GetStyle().Alpha * alpha);
	...
	bool command_palette()
	{
		...
		else
		{
			for (int visible_index = 0; visible_index < static_cast<int>(palette.filtered_indices.size()); ++visible_index)
			{
				...
				{
					row_bg = background_hovered_color();
				}
				if (selected || hovered)
				{
					draw->AddRectFilled(min, max, col_u32(row_bg), imgui_frame_rounding());
				}
...

		{
			...
			const float inset = Math::max(1.0f, imgui_border_size());
	...
	{
		const Vec4 c     = has_color(color) ? color : accent_color();
	...
	bool begin_toolbar(StringView id)
	{
		...
		options.size = Size(0.0f, imgui_frame_height() + imgui_frame_padding() * 2.0f);
	...
	void callout(StringView title, StringView message, NotificationKind kind)
	{
		...
		const Vec4 background  = mix_color(panel_color(), accent, 0.08f);
		const Vec4 border      = mix_color(imgui_color(ImGuiCol_Border), accent, 0.35f);
		...
		draw->AddRect(pos, max, col_u32(border), rounding, 0, imgui_border_size());
		...
		if (has_text(title_text))
		{
			draw_list_add_text(draw, ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(text_x, pos.y + padding),
			                   col_u32(imgui_color(ImGuiCol_Text)), title_text, wrap_w);
		}
		if (has_text(message_text))
		{
			...
			draw_list_add_text(draw, ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(text_x, message_y),
			                   col_u32(text_muted_color()), message_text, wrap_w);
		}
	}
	...
	void banner(StringView title, StringView message, const Vec4& accent_color)
	{
		...
		const float rounding   = imgui_window_rounding();
		const float padding    = imgui_window_padding() * 1.35f;
		const float spacing    = imgui_item_spacing();
		const float strip_size = 5.0f;
		const Vec4 accent      = has_color(accent_color) ? accent_color : UI::accent_color();
		const Vec4 background  = mix_color(panel_color(), accent, 0.10f);
		const Vec4 border      = mix_color(imgui_color(ImGuiCol_Border), accent, 0.45f);
	...
	bool hero(const HeroOptions& options)
	{
		...
		const float rounding      = options.rounding >= 0.0f ? options.rounding : imgui_window_rounding();
		const float padding       = options.padding >= 0.0f ? options.padding : imgui_window_padding() * 2.0f;
		...
		const Vec4 background     = mix_color(panel_color(), accent, 0.04f);
		const Vec4 border         = mix_color(imgui_color(ImGuiCol_Border), accent, 0.25f);
		...
		if (has_text(action_text))
		{
			...
			button_w               = Math::max(96.0f, action_sz.x + imgui_frame_padding() * 2.0f);
			button_h               = imgui_frame_height();
		...
		if (options.border)
		{
			draw->AddRect(pos, max, col_u32(border, alpha_mul), rounding, 0, imgui_border_size());
		}
		...
		if (has_text(title_text))
		{
			draw_list_add_text(draw, ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(block_x(title_sz.x), y),
			                   col_u32(imgui_color(ImGuiCol_Text), alpha_mul), title_text, content_width);
		...
		if (has_text(description_text))
		{
			draw_list_add_text(draw, ImGui::GetFont(), ImGui::GetFontSize(), ImVec2(block_x(desc_sz.x), y),
			                   col_u32(text_muted_color(), alpha_mul), description_text, desc_wrap);
			y += desc_sz.y + spacing;
		}
	...
	void error_state(StringView message, StringView title)
	{
		...
		draw->AddRect(pos, add(pos, size), col_u32(active_context()->style.colors.error, 0.7f), imgui_frame_rounding());
		...
		draw_list_add_text(draw, ImVec2(pos.x + 14.0f, pos.y + 34.0f), col_u32(imgui_color(ImGuiCol_Text)),
